...
**Required post-deploy step for the career-agnostic onboarding change, NOT YET PERFORMED.** The fixture tests (`lib/__fixtures__/fit-prompt.*.txt`) pin the prompt BUILDER against fixed synthetic inputs — they cannot prove the values the existing user actually pastes at onboarding match the hardcoded text they replace. That proof requires a database and a completed onboarding run in production, and this repo has neither: no `.env.local`, and Tom Keefe has not yet run the onboarding flow against production. Once he does, render the live fit prompt before and after and diff it:
...
Expected: no diff, or a diff accountable line by line. The pasted values should match `lib/search-criteria.ts`'s old `DEFAULT_FIT_BRAIN` and `lib/fit-prompt.ts`'s `DEFAULT_TITLE_SCOPE` / `DEFAULT_DOMAIN_BONUS` — hand those over as text from git history, never retyped:
...
`readOnboardedAtFor(tenantId)` in `lib/settings-store.ts` takes its tenant EXPLICITLY and must keep doing so — this is the single most tempting "tidy-up" in the codebase, and it must be warned against by name. Every other reader in that file calls `resolveTenantId()` (`lib/tenant.ts`), which branches on `isPlatform()` first — the crawler's platform context resolves to whatever tenant `runAsTenant()` set, or throws if none was set, with no session involved at all — and only in the non-platform case is it `(await requireActor()).tenantId`; `readOnboardedAtFor`'s only caller is `requireActorPage()` itself, so a version that resolved its own tenant would call `requireActor()` from inside `requireActor()`'s own module-level flow — unbounded recursion no type check catches.

**The guard, and its limit.** `lib/career-neutrality.test.ts` has three checks. Two fail if any production module outside `lib/profile.ts` / `lib/fit-prompt.ts` holds one of the nine career-specific phrases extracted into the profile (`searchSubject`, `querySubject`, `stackFamilyIntro`, `candidatePersona`, `buildingConcept`, `buildingUpside`, and the three scoring-guide tails), or imports one of six now-deleted GTM constant names (`SEARCH_SUBJECT`, `QUERY_SUBJECT`, etc.). The third is unrelated to the profile fields: it fails if any file under `app/` or `components/` names the previous owner (`\bTom\b`, case-insensitive) — the check most likely to trip on a future edit that pastes in example copy. Any field added to `Profile` belongs in the first two. **But the phrase checks catch only the strings that were extracted** — three career-specific, user-visible strings were found by hand instead, during this same work, and none of them were things the guard could see: a `"Denver/CO GTM / RevOps"` phrase in a Discover empty-state message, a `"GTM stack"` dropdown label in `RoleSearchPanel`, and a `"GTM stack terms"` tooltip in `RolesTable` — all three since fixed, none present in the current codebase. It proves the switch is complete for the strings this project identified; it does not prove the app never assumes a career.
...
- **Company-name variant merging.** `normalizeCompanyName` (`lib/role-key.ts`) is lowercase/trim/whitespace-collapse only — no aliasing. Real probe data returned RTX under two different spellings, `"RTX (Raytheon)"` and `"Raytheon (RTX)"`, and `getAllDiscoveredStartups` in `app/actions/discover.ts` still renders those as two separate Discover cards, each with its own signal list. Fixing it needs a real "same company" decision (substring match? a canonical-name lookup?), not a bigger normalizer.
- **Watchlist shows nothing for a non-venture row's signal.** `withLegacyExtraFields` in `app/actions/discover.ts` keeps `app/actions/watchlist.ts` / `components/Watchlist.tsx` compiling against the old six-field shape. `Watchlist.tsx` still renders `c.company`, `c.tagline`, and `c.source` fine, but its only signal-shaped tags are `c.stage` / `c.raised` / `c.category` (`components/Watchlist.tsx:168-172`) — the load-bearing gap is that `TrackedCompany` (`lib/types.ts`) has no `signal` or `extras` field at all, so a defence contractor's or a hospital's actual hiring signal, which lives in `extras` under keys their own `HiringSignal.extraFields` named, never reaches this page in any form. Needs a `TrackedCompany` schema change, not a rendering fix.
- **Discover's prompt dropped an exclusion clause with nothing replacing it.** The old hardcoded system prompt said "Focus exclusively on Series B and above (Series B, Series C, Series D+, Late Stage, Growth, Pre-IPO). Exclude seed, pre-seed, and Series A rounds." The new `hiringSignalSystem()` (`lib/hiring-signal-prompt.ts`) renders only `qualifier` ("Series B and above") — the floor, with no exclusion. Documented as a ruling at the top of that file: a deliberate, accepted behaviour change (the existing user's Discover result set can widen to include a round the model judges "close enough"), not a regression to fix silently.
