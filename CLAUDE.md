...
**The prompt-generalisation pass is not finished** — it deliberately stopped at the fit prompt and the search/extraction prompts. Still hardcoded to venture-backed-tech vocabulary rather than parameterized: the `FINANCIAL SIGNALS` block in `lib/fit-prompt.ts` (the ARR thresholds, PE-exit language, and "a16z, Sequoia, Benchmark" backer list) and the `ARR:` / `Backer:` / `Exit signal:` lines it reads in the role block; `roleExtractionSchema`'s `seniority` enum (`"VP/Head"`, `"Director"`, `"Senior Manager"`, `"Manager/IC"`) in `lib/search-criteria.ts`; and `app/actions/discover.ts`'s funding-analyst system prompt and search prompt, which are the single largest block of venture vocabulary in the codebase. All of these were left alone deliberately, not missed: each is guarded ("only if the candidate cares", "absence of this data is not a deduction"), so for a non-GTM user they degrade quietly — the model is told to ignore what it can't find — rather than actively breaking. That's a real deferral, not a gap in coverage.
