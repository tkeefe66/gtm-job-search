...
**`app/actions/discover.ts` is a different case and must not be filed under that guarantee.** Its funding-analyst system prompt and per-window search prompt are the single largest block of venture vocabulary in the codebase, and NOTHING in them is guarded: the system prompt says "You are a startup funding analyst… Focus exclusively on Series B and above", and the search prompt hard-searches TechCrunch/Crunchbase/Bloomberg for AI startup funding rounds. For a mechanical engineer or a nurse this does not degrade quietly — it returns venture-backed AI startups, confidently and every time. It is out of scope for the prompt-generalisation pass because the whole feature needs rethinking (what public event predicts hiring in a given field?), not because it is harmless.
