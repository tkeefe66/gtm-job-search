...
- **Watchlist shows nothing for a non-venture row.** `withLegacyExtraFields` in `app/actions/discover.ts` keeps `app/actions/watchlist.ts` / `components/Watchlist.tsx` compiling against the old six-field shape, but `Watchlist.tsx` renders only `c.stage` / `c.raised` / `c.category` (`components/Watchlist.tsx:170-172`) — all empty for a defence contractor or a hospital, whose signal lives in `extras` under keys their own `HiringSignal.extraFields` named. The tenant's hiring `signal` line never reaches that page. Needs a `TrackedCompany` schema change (`lib/types.ts`), not a rendering fix.
