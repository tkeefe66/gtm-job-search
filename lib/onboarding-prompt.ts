...
- hiringSignal (object): { name, sources (array of strings), qualifier, exclusions, hasRecency (boolean), extraFields (array of strings) } — a PUBLIC EVENT OR PROPERTY that predicts an employer in this field is about to hire, plus the kinds of publications or registries where such news or facts appear. Set hasRecency to true when it is a dated EVENT (a funding round, a contract award, a plant opening, a new facility license) and false when it is a standing PROPERTY of the employer (holding a certain accreditation, operating a certain kind of facility) rather than something that just happened.
  ...
  - exclusions (string): what sits BELOW the bar, named outright — the complement of qualifier, used to tell the model what to throw away rather than only what to aim at. An empty string is a valid answer when nothing needs ruling out. Funding example: "seed, pre-seed, and Series A rounds". Engineering example: "task orders, IDIQ ceiling announcements, and re-competes of existing work". Keep it to a short list of kinds, not a sentence.
