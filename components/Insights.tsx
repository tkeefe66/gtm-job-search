...
import { analyzePipeline } from "@/app/actions/insights";
...
export default function Insights({
  ...
  async function run() {
    ...
    if (res.insights) setData(res.insights);
    setLoading(false);
  }
  ...
  return (
    <div>
      ...
      {loading && (
        <div className="py-12">
          <Spinner label="Analyzing your pipeline and the market…" />
        </div>
      )}
        ...
        <div className="flex flex-col gap-6">
          <Section title="What the market wants">
            <div className="flex flex-wrap gap-2">
              {data.top_skills_in_demand.map((s, i) => (
                <Tag key={i}>{s}</Tag>
              ))}
            </div>
          </Section>
